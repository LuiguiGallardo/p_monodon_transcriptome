...
if (file.exists("diffExpr.P1e-3_C2.matrix.RData") && ! NO_REUSE) {
    ...
    primary_data = read.table("diffExpr.P1e-3_C2.matrix", header=T, com='', row.names=1, check.names=F)
...
samples_data = read.table("../metadata_trinity.txt", header=F, check.names=F, fill=T)
